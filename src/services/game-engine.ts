...
// Initial named leaders per region (personality determines action effectiveness modifier)
const REGION_LEADERS: Record<GameRegionId, GameLeader> = {
  northAmerica:     { name: 'Pres. Margaret Holt',     personality: 'pragmatist' },
  europe:           { name: 'Chancellor Erik Hoffmann', personality: 'reformist'  },
  eastAsia:         { name: 'Gen. Sec. Zhang Wei',      personality: 'nationalist' },
  southAsia:        { name: 'PM Priya Anand',           personality: 'pragmatist' },
  mena:             { name: 'Supreme Leader Al-Rashid', personality: 'hawk'       },
  subSaharanAfrica: { name: 'Pres. Kofi Mensah',        personality: 'populist'   },
  latam:            { name: 'Pres. Carlos Vega',        personality: 'populist'   },
  centralAsia:      { name: 'Dir. Viktor Sorokin',      personality: 'hawk'       },
  oceania:          { name: 'PM Sarah Lennox',          personality: 'dove'       },
};

/** Leader personality modifier table (multiplies action effectiveness). */
const PERSONALITY_MODIFIERS: Record<LeaderPersonality, Record<GameActionCategory, number>> = {
  hawk:        { diplomatic: 0.7, economic: 0.8, military: 1.3, covert: 1.2 },
  dove:        { diplomatic: 1.4, economic: 1.3, military: 0.6, covert: 0.7 },
  reformist:   { diplomatic: 1.2, economic: 1.3, military: 0.8, covert: 0.8 },
  pragmatist:  { diplomatic: 1.0, economic: 1.0, military: 1.0, covert: 1.0 },
  nationalist: { diplomatic: 0.6, economic: 0.9, military: 1.2, covert: 1.1 },
  populist:    { diplomatic: 1.1, economic: 1.2, military: 0.8, covert: 0.9 },
};

const ALL_PERSONALITIES: LeaderPersonality[] = ['hawk', 'dove', 'reformist', 'pragmatist', 'nationalist', 'populist'];
...
/**
 * Diplomatic reaction watchers — which regions issue reactions to major actions
 * taken in a given region. Represents geopolitical interest and proximity.
 */
const REACTION_WATCHERS: Record<GameRegionId, GameRegionId[]> = {
  northAmerica:     ['europe', 'latam'],
  europe:           ['northAmerica', 'centralAsia'],
  eastAsia:         ['southAsia', 'oceania', 'northAmerica'],
  southAsia:        ['eastAsia', 'mena'],
  mena:             ['europe', 'centralAsia', 'southAsia'],
  subSaharanAfrica: ['europe', 'mena'],
  latam:            ['northAmerica', 'europe'],
  centralAsia:      ['europe', 'eastAsia', 'southAsia'],
  oceania:          ['northAmerica', 'eastAsia'],
};

/** Technology Level thresholds that unlock advanced actions. */
const TL_UNLOCK: Record<string, number> = {
  satelliteSurveillance: 70,
  aiDisinformation:      80,
  cyberDeterrence:       90,
};
...
interface EventTemplate {
  ...
  cascade?: boolean;
  /** Turns before this crisis auto-resolves if ignored (undefined = no deadline). */
  deadline?: number;
  /** ID of a follow-up template that fires after chainDelay turns. */
  chainTemplateId?: string;
  /** How many turns after this event before the chain fires (default 2). */
  chainDelay?: number;
}
...
const EVENT_TEMPLATES: EventTemplate[] = [
  // -- Crisis events (Shadow President staples) --
  {
    ...
    defconDelta: -1,
    deadline: 3,
    chainTemplateId: 'retaliatory-cyber',
    chainDelay: 1,
  },
  {
    id: 'military-tensions',
  ...
  },
  {
    ...
    cascade: true,
    deadline: 2,
    chainTemplateId: 'sanctions-ultimatum',
    chainDelay: 2,
  },
  {
    id: 'coup-attempt',
    ...
    approvalDelta: -3,
    cascade: true,
    chainTemplateId: 'post-coup-instability',
    chainDelay: 2,
  },
  {
    id: 'terrorist-attack',
  ...
  },
  {
    ...
    approvalDelta: -8,
    chainTemplateId: 'diplomatic-expulsion',
    chainDelay: 1,
  },
  {
    ...
    cascade: true,
    deadline: 3,
    chainTemplateId: 'refugee-wave',
    chainDelay: 2,
  },
  // -- Positive events --
  {
    id: 'diplomatic-summit-trade',
  ...
  },
  {
    ...
    cascade: true,
    deadline: 3,
    chainTemplateId: 'economic-recession',
    chainDelay: 3,
  },
  {
    id: 'arms-deal',
  ...
  },
  {
    ...
    approvalDelta: 5,
    defconDelta: 1,
    chainTemplateId: 'reconstruction-request',
    chainDelay: 2,
  },
  {
    id: 'assassination',
    ...
    defconDelta: -1,
    cascade: true,
    chainTemplateId: 'succession-crisis',
    chainDelay: 2,
  },
  {
    id: 'trade-war',
    ...
    approvalDelta: -4,
    cascade: true,
    chainTemplateId: 'supply-chain-crisis',
    chainDelay: 2,
  },
  {
    id: 'crackdown',
  ...
  // -- Chain follow-up templates (never selected as primary events) --
  {
    id: 'sanctions-ultimatum',
    headline: 'International community demands sanctions compliance',
    description: 'Following the nuclear test, world powers issue a joint ultimatum demanding immediate disarmament talks.',
    regions: ['eastAsia', 'centralAsia', 'mena'],
    impact: r => ({ [r]: { influence: -6, stability: -5 } }),
    resourceDelta: { politicalCapital: -5 },
    approvalDelta: -2,
    defconDelta: -1,
  },
  {
    id: 'post-coup-instability',
    headline: 'Post-coup power struggle destabilises the region',
    description: 'Factional fighting continues as the new government struggles to consolidate control.',
    regions: ['mena', 'subSaharanAfrica', 'latam', 'centralAsia'],
    impact: r => ({ [r]: { stability: -12, threatLevel: 10 } }),
    approvalDelta: -3,
    cascade: true,
  },
  {
    id: 'economic-recession',
    headline: 'Pandemic fallout triggers deep economic recession',
    description: 'GDP contracts sharply as supply chains buckle and consumer confidence collapses.',
    regions: ['southAsia', 'subSaharanAfrica', 'latam', 'eastAsia'],
    impact: r => ({ [r]: { stability: -8, influence: -4 } }),
    resourceDelta: { economicInfluence: -10 },
    approvalDelta: -4,
    cascade: true,
  },
  {
    ...
    headline: 'Global supply chain crisis halts key industries',
    description: 'Shortages of critical components shut down factories worldwide as logistics networks buckle.',
    regions: ['northAmerica', 'europe', 'eastAsia'],
    impact: r => ({ [r]: { stability: -6, influence: -3 } }),
    resourceDelta: { economicInfluence: -8 },
    approvalDelta: -3,
  },
  {
    id: 'reconstruction-request',
    headline: 'Devastated region requests urgent reconstruction aid',
    description: 'Local authorities issue an international appeal for infrastructure rebuilding funds and technical expertise.',
    regions: ['mena', 'subSaharanAfrica', 'southAsia', 'centralAsia', 'latam'],
    impact: r => ({ [r]: { stability: 4, influence: 5 } }),
    resourceDelta: { politicalCapital: -4, economicInfluence: -6 },
    approvalDelta: 2,
  },
  {
    id: 'refugee-wave',
    headline: 'Refugee migration wave strains neighbouring states',
    description: 'Hundreds of thousands flee conflict and famine, overwhelming host country resources.',
    regions: ['europe', 'southAsia', 'subSaharanAfrica', 'mena'],
    impact: r => ({ [r]: { stability: -8, threatLevel: 6, influence: -3 } }),
    resourceDelta: { politicalCapital: -5 },
    approvalDelta: -3,
    cascade: true,
  },
  {
    id: 'retaliatory-cyber',
    headline: 'Retaliatory cyber strike hits government networks',
    description: 'A counter-attack disables key government databases and public-facing digital services.',
    regions: ['europe', 'eastAsia', 'northAmerica'],
    impact: r => ({ [r]: { stability: -6, threatLevel: 8 } }),
    resourceDelta: { intelligenceAssets: -4 },
    approvalDelta: -3,
  ...
  {
    id: 'diplomatic-expulsion',
    headline: 'Mass diplomatic expulsions follow intelligence scandal',
    description: 'Several countries expel each other\'s envoys in tit-for-tat moves following leaked intelligence files.',
    regions: ['northAmerica', 'europe', 'mena'],
    impact: r => ({ [r]: { influence: -8, stability: -3 } }),
    resourceDelta: { politicalCapital: -6 },
    approvalDelta: -4,
  },
  {
    id: 'succession-crisis',
    headline: 'Political vacuum sparks succession crisis',
    description: 'Rival factions manoeuvre for power following the sudden leadership change, risking prolonged instability.',
    regions: ['mena', 'centralAsia', 'subSaharanAfrica', 'latam'],
    impact: r => ({ [r]: { stability: -10, influence: -5, threatLevel: 8 } }),
    approvalDelta: -3,
    cascade: true,
  },
];
...
export function getAvailableActions(state: GameState): GameAction[] {
  ...
  for (const regionId of regions) {
    ...
    // ── Covert ──────────────────────────────────────────────────────
    const tl = state.resources.technologyLevel;
    actions.push(
      ...
      // Tech-unlocked covert actions
      {
        type: 'satelliteSurveillance', category: 'covert',
        label: `Satellite Surveillance → ${n}`,
        description: 'Deploy orbital assets to monitor military movements and reduce threat level.',
        targetRegion: regionId, cost: { technologyLevel: 12, intelligenceAssets: 8 }, risk: 15, approvalImpact: 1,
        locked: tl < TL_UNLOCK['satelliteSurveillance']!, unlockRequirement: 'Requires Technology Level 70',
      },
      {
        type: 'aiDisinformation', category: 'covert',
        label: `AI Disinformation → ${n}`,
        description: 'Deploy AI-driven influence operations with precision targeting and lower exposure risk.',
        targetRegion: regionId, cost: { technologyLevel: 15, intelligenceAssets: 10 }, risk: 20, approvalImpact: -3,
        locked: tl < TL_UNLOCK['aiDisinformation']!, unlockRequirement: 'Requires Technology Level 80',
      },
    );
    // Tech-unlocked military action
    actions.push({
      type: 'cyberDeterrence', category: 'military',
      label: `Cyber Deterrence → ${n}`,
      description: 'Demonstrate offensive cyber capability to deter escalation. Reduces DEFCON pressure.',
      targetRegion: regionId, cost: { technologyLevel: 20, militaryReadiness: 15 }, risk: 25, approvalImpact: 4,
      locked: tl < TL_UNLOCK['cyberDeterrence']!, unlockRequirement: 'Requires Technology Level 90',
    });
  }
  return actions;
}
...
// IDs of templates that only fire as chain follow-ups (excluded from random selection)
const CHAIN_ONLY_IDS = new Set<string>([
  'sanctions-ultimatum', 'post-coup-instability', 'economic-recession', 'supply-chain-crisis',
  'reconstruction-request', 'refugee-wave', 'retaliatory-cyber', 'diplomatic-expulsion', 'succession-crisis',
]);

// Indices of primary (selectable) templates
const PRIMARY_TEMPLATE_INDICES: number[] = EVENT_TEMPLATES
  .map((t, i) => ({ id: t.id, i }))
  .filter(({ id }) => !CHAIN_ONLY_IDS.has(id))
  .map(({ i }) => i);

/**
 * Generate 1-3 world events for the current turn.
 * Also processes any pending chain events due this turn.
 */
export function generateTurnEvents(state: GameState, seed?: number): GameEvent[] {
  ...
  // Drain pending chain events that are due this turn
  const dueChains = state.pendingChainEvents.filter(c => c.triggerTurn <= state.turn);
  state.pendingChainEvents = state.pendingChainEvents.filter(c => c.triggerTurn > state.turn);
  for (const chain of dueChains) {
    const tmpl = EVENT_TEMPLATES.find(t => t.id === chain.templateId);
    if (!tmpl) continue;
    const region: GameRegionId = tmpl.regions.includes(chain.originRegion)
      ? chain.originRegion
      : (tmpl.regions[0] ?? chain.originRegion);
    const regionState = state.regions[region];
    const briefings: AdvisorBriefing[] = state.advisors.map(a => ({
      advisorId: a.id,
      text: ADVISOR_TEMPLATES[a.id](placeholderEvent(state.turn, events.length, tmpl, region), regionState),
    }));
    events.push({
      id: `chain-${state.turn}-${events.length}`,
      turn: state.turn,
  ...
  for (let i = 0; i < count; i++) {
    // Pick a primary template not already used this turn
    let pick = Math.floor(rand() * PRIMARY_TEMPLATE_INDICES.length);
    while (usedIndices.has(pick)) {
      pick = (pick + 1) % PRIMARY_TEMPLATE_INDICES.length;
    }
    usedIndices.add(pick);
    const tmplIdx = PRIMARY_TEMPLATE_INDICES[pick]!;
    ...
    const evt: GameEvent = {
      id: `evt-${state.turn}-${events.length}`,
      turn: state.turn,
      headline: tmpl.headline,
      description: tmpl.description,
      region,
    ...
    if (tmpl.deadline != null) evt.deadline = tmpl.deadline;
    events.push(evt);
  }
  return events;
}
 ...
 * Also queues deadline events and chain follow-ups.
 */
export function applyEvents(state: GameState, events: GameEvent[]): GameState {
  for (const evt of events) {
    state.log.push(evt);

    // Track deadline events for pending resolution (only on turns after they were shown)
    // They're added to pendingEvents at end-of-turn, not immediately, so they don't render twice
    if (evt.deadline != null && !evt.autoResolved && !evt.isChained) {
      state.pendingEvents.push({ ...evt });
    }

    // Queue chain follow-up events
    const tmpl = EVENT_TEMPLATES.find(t => t.headline === evt.headline && !evt.isChained);
    if (tmpl?.chainTemplateId) {
      const delay = tmpl.chainDelay ?? 2;
      const alreadyQueued = state.pendingChainEvents.some(c => c.templateId === tmpl.chainTemplateId && c.originRegion === evt.region);
      if (!alreadyQueued) {
        state.pendingChainEvents.push({
          templateId: tmpl.chainTemplateId,
          triggerTurn: state.turn + delay,
          originRegion: evt.region,
        });
      }
    }
...
export function resolveAction(state: GameState, action: GameAction, seed?: number): GameEvent {
  // Locked actions cannot be executed
  if (action.locked) {
    throw new Error(`Action ${action.type} is locked: ${action.unlockRequirement ?? 'technology level too low'}`);
  }
  ...
  if (action.type === 'cyberDeterrence') {
    state.defcon = clamp(state.defcon + 1, 1, 5) as DefconLevel;
  }
  ...
  if (action.category === 'covert' && rand() * 100 < effectiveRisk) {
    ...
    exposureText = ' Operation EXPOSED — approval and influence damaged.';
  ...
  if (action.category === 'military' && action.type !== 'withdrawTroops' && action.type !== 'cyberDeterrence') {
  ...
  // Leader change after a coup (new random personality)
  if (action.type === 'fundCoup' && !wasExposed) {
    const newPersonality = ALL_PERSONALITIES[Math.floor(rand() * ALL_PERSONALITIES.length)] ?? 'pragmatist';
    region.leader = { name: generateCoupLeaderName(rand), personality: newPersonality };
  }

  // Clear pending deadline events for this region (player responded)
  state.pendingEvents = state.pendingEvents.filter(e => e.region !== action.targetRegion);

  const evt: GameEvent = {
    id: `act-${state.turn}-${actionIdx}`,
    turn: state.turn,
    headline: `Action: ${action.label}`,
    description: action.description + (exposureText ? ` ⚠️${exposureText}` : ''),
  ...
  // Diplomatic reactions from third-party regions for high-profile visible actions
  const REACTION_TRIGGERS: GameAction['type'][] = ['imposeSanctions', 'deployTroops', 'fundCoup', 'diplomaticThreaten', 'nuclearPosture'];
  if (REACTION_TRIGGERS.includes(action.type) && (!wasExposed || action.category !== 'covert')) {
    const reactions = generateDiplomaticReactions(state, action, actionIdx, rand);
    for (const r of reactions) state.log.push(r);
  }

  return evt;
}

/** Generate a plausible post-coup leader name. */
function generateCoupLeaderName(rand: () => number): string {
  const firstNames = ['Gen.', 'Col.', 'Adm.', 'Dir.', 'Cdr.'];
  const lastNames = ['Morales', 'Petrov', 'Hassan', 'Nguyen', 'Osei', 'Kowalski', 'Reyes', 'Zhao', 'Ibrahim', 'Santos'];
  return `${firstNames[Math.floor(rand() * firstNames.length)]} ${lastNames[Math.floor(rand() * lastNames.length)]}`;
}

/**
 * Generate diplomatic reaction events from third-party regions following a major visible action.
 */
function generateDiplomaticReactions(
  state: GameState,
  action: GameAction,
  actionIdx: number,
  rand: () => number,
): GameEvent[] {
  const watchers = REACTION_WATCHERS[action.targetRegion] ?? [];
  if (watchers.length === 0) return [];

  // Pick 1 watcher to react (not all, to avoid flooding the log)
  const reactorId = watchers[Math.floor(rand() * watchers.length)];
  if (!reactorId) return [];
  const reactor = state.regions[reactorId];
  if (!reactor) return [];

  const targetRegion = state.regions[action.targetRegion];
  const isHostileAction = ['imposeSanctions', 'deployTroops', 'fundCoup', 'nuclearPosture'].includes(action.type);

  // Watcher reaction depends on relationship with target
  // Allies of the target condemn hostile actions; rivals welcome them
  const reactorAlliedWithTarget = reactor.influence > 20 && targetRegion.influence > 20;
  const condemnAction = isHostileAction && reactorAlliedWithTarget;
  const supportAction = isHostileAction && !reactorAlliedWithTarget && reactor.influence < 0;

  let headline: string;
  let description: string;
  let approvalDelta: number;
  let influenceDelta: number;

  if (action.type === 'diplomaticThreaten') {
    headline = `${reactor.name} calls for de-escalation following ultimatum`;
    description = `${reactor.name} urges restraint after the public ultimatum directed at ${targetRegion.name}.`;
    approvalDelta = -1;
    influenceDelta = -2;
  } else if (condemnAction) {
    headline = `${reactor.name} condemns intervention in ${targetRegion.name}`;
    description = `${reactor.name} issues a formal protest, calling the action an unacceptable violation of sovereignty.`;
    approvalDelta = -2;
    influenceDelta = -3;
  } else if (supportAction) {
    headline = `${reactor.name} signals support for pressure on ${targetRegion.name}`;
    description = `${reactor.name} welcomes the move, citing shared concerns about regional instability.`;
    approvalDelta = 1;
    influenceDelta = 3;
  } else {
    headline = `${reactor.name} monitors situation in ${targetRegion.name}`;
    description = `${reactor.name} is closely watching developments, urging all parties to exercise caution.`;
    approvalDelta = 0;
    influenceDelta = -1;
  }

  // Apply influence change to reacting region
  reactor.influence = clamp(reactor.influence + influenceDelta, -100, 100);
  state.approval = clamp(state.approval + approvalDelta, 0, 100);

  return [{
    id: `react-${state.turn}-${actionIdx}`,
    turn: state.turn,
    headline,
    description,
    region: reactorId,
    impact: { [reactorId]: { influence: influenceDelta } },
    approvalDelta,
    isReaction: true,
    reactingRegion: reactorId,
  }];
}

/**
 * Decrement deadline counters on pending events. Auto-resolve expired ones at 1.5× impact.
 * Called at the start of each new turn (resolution → briefing transition).
 */
export function processPendingDeadlines(state: GameState): void {
  const stillPending: GameEvent[] = [];
  for (const evt of state.pendingEvents) {
    const remaining = (evt.deadline ?? 1) - 1;
    if (remaining <= 0) {
      // Auto-resolve: apply 1.5× impact
      const autoEvt: GameEvent = {
        ...evt,
        id: `auto-${evt.id}`,
        turn: state.turn,
        headline: `UNADDRESSED: ${evt.headline}`,
        description: `The crisis in ${state.regions[evt.region]?.name ?? evt.region} escalated unchecked — consequences are severe.`,
        autoResolved: true,
        deadline: undefined,
      };
      // Apply scaled impact
      for (const [rId, delta] of Object.entries(autoEvt.impact) as [GameRegionId, { influence?: number; stability?: number; threatLevel?: number }][]) {
        const r = state.regions[rId];
        if (!r) continue;
        if (delta.influence   != null) r.influence   = clamp(r.influence   + Math.round(delta.influence   * 1.5), -100, 100);
        if (delta.stability   != null) r.stability   = clamp(r.stability   + Math.round(delta.stability   * 1.5), 0, 100);
        if (delta.threatLevel != null) r.threatLevel = clamp(r.threatLevel + Math.round(delta.threatLevel * 1.5), 0, 100);
      }
      if (autoEvt.approvalDelta) state.approval = clamp(state.approval + Math.round(autoEvt.approvalDelta * 1.5), 0, 100);
      if (autoEvt.defconDelta)   state.defcon   = clamp(state.defcon + autoEvt.defconDelta, 1, 5) as DefconLevel;
      state.log.push(autoEvt);
    } else {
      stillPending.push({ ...evt, deadline: remaining });
    }
  }
  state.pendingEvents = stillPending;
}
...
function actionEffects(action: GameAction, region: GameRegionState): { influence: number; stability: number; threatLevel: number } {
  ...
  // Leader personality modifier (stacked on top of government type)
  const personalityMod = leaderModifier(region.leader.personality, action.category);

  const base = baseActionEffects(action.type);
  const combined = govMod * personalityMod;
  return {
    influence:   Math.round(base.influence   * combined),
    stability:   Math.round(base.stability   * combined),
    threatLevel: Math.round(base.threatLevel * combined),
  };
}

/**
 * Leader personality modifier — multiplies action effectiveness based on the leader's disposition.
 */
function leaderModifier(personality: LeaderPersonality, category: GameActionCategory): number {
  return PERSONALITY_MODIFIERS[personality]?.[category] ?? 1.0;
}
...
function baseActionEffects(type: GameAction['type']): { influence: number; stability: number; threatLevel: number } {
  switch (type) {
    ...
    // Tech-unlocked actions
    case 'satelliteSurveillance': return { influence:  4, stability:  2, threatLevel: -12 };
    case 'aiDisinformation':      return { influence: 10, stability: -2, threatLevel:  -2 };
    case 'cyberDeterrence':       return { influence:  2, stability:  3, threatLevel:  -8 };
